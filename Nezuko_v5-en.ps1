...
function Initialize-System {
    ...
    foreach ($l in $letters) {
        ...
        Write-Host "  [OK] ${l}: - Initialized" -ForegroundColor Green
    }
    ...
    Write-Host ""
    Write-Host "  [OK] System Ready - Any 2 of $($letters.Count) drives unlock the key" -ForegroundColor Green
...
function Get-MasterKey {
    # ================================================================
    # Search by content only - Drive letter/label does not matter.
    # Any 2 drives with nzk_keys.bin whose keys match the ledger
    # will unlock the Master Key automatically.
    # ================================================================
    ...
    # --- Step 1: Scan for drives containing nzk_keys.bin ---
    $foundDrives = @()
    ...
    # --- Step 2: Test drive pairs until a matching pair is found ---
        ...
        for ($j = $i+1; $j -lt $foundDrives.Count; $j++) {
            ...
            foreach ($pname in $ledger.pair_ciphers.Keys) {
                ...
                try {
                    ...
                    Write-Host "  [OK] Key unlocked using pair: [$pname]" -ForegroundColor Green
...
function Replace-Drive {
    # ================================================================
    # Works by content only - No need to know old drive letter
    # ================================================================
    Write-Host ""
    ...
    foreach ($l in $allLetters) {
        ...
        if (Test-Path $kp) {
            try {
                ...
                Write-Host "  ${l}: [OK] Has key file" -ForegroundColor Green
            } catch {}
        }
    }
    ...
    Write-Host ""
    Write-Host "  [OK] Copied $copiedCount pair keys to ${newLetter}:" -ForegroundColor Green
...
function Add-Manual {
    ...
    Write-Host "  [OK] Fossilized: $name" -ForegroundColor Green
}
...
function Verify-All {
    ...
    foreach ($name in $ledger.files.Keys) {
        ...
        if ($ledger.files[$name].signature -eq $sig) {
            Write-Host "  [OK] Valid:   $name" -ForegroundColor Green; $ok++
        } else {
...
function Show-Status {
    ...
    foreach ($l in $allLetters) {
        ...
        if (Test-Path $keyPath) {
            ...
            Write-Host "  ${l}: [OK] Has key file ($sizekb KB)" -ForegroundColor Green
    ...
    if ($foundDrives.Count -ge 2) {
        Write-Host "  System Ready - can unlock with any 2 drives [OK]" -ForegroundColor Green
