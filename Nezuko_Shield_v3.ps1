...
Get-ChildItem -Path $source -File | ForEach-Object {
    ...
    # ?? ãáÝ ÈÇãÊÏÇÏ (ÕæÑÉ¡ ÝíÏíæ¡ php...) ??
    # áÇ íõÓÊÈÏá ÃÈÏÇð — ÝÞØ ÇÍÝÙ äÓÎÉ ÌÏíÏÉ ÈÊÇÑíÎ ÅÐÇ ãÎÊáÝ
    if ($hasExtension) {
        if ($sSize -eq $dSize) {
            ...
            # ÍÌã ãÎÊáÝ = ãáÝ ãÎÊáÝ = ÇÍÝÙ ÈÌÇäÈå ÈÊÇÑíÎ
            $stamp   = Get-DateStamp $sFile
            $newName = "${baseName}_${stamp}${ext}"
    ...
    # ?? ãáÝ ÈÏæä ÇãÊÏÇÏ (Google Studio docs) ??
    # ÞÇÚÏÉ Çáäãæ ÇáÐåÈíÉ: íßÈÑ = ÍÏøË / íÕÛÑ = ÇÍãö
    if ($sSize -gt $dSize) {
